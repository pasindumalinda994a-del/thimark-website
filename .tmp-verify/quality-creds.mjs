...
try {
  ...
  const info = await desktop.evaluate(() => {
    const creds = [...document.querySelectorAll(".quality-cred")];
    return creds.map((c) => {
      ...
      return {
        text: c.getAttribute("aria-label"),
  ...
  await desktop.screenshot({ path: path.join(dir, "quality-creds-desktop.png") });
  const foot = desktop.locator("#quality");
  const box = await foot.boundingBox();
  if (box) {
    await desktop.screenshot({
      path: path.join(dir, "quality-creds-desktop-crop.png"),
      clip: {
        x: Math.max(0, box.x),
  ...
  await mobile.screenshot({ path: path.join(dir, "quality-creds-mobile.png") });
